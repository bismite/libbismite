...
int main(int argc, char* argv[])
{
  ...
  set_texture(&layer_a->root,"assets/check.png");
  layer_a->textures[0] = layer_a->root.texture;
  // Sprites for Layer A
  BiNode* tester = make_sprite("assets/tester.png");
  bi_node_set_position(tester,480/4,320/2);
  bi_node_add_node(&layer_a->root,tester);
  ...
  bi_node_add_node(&layer_b->root, tester2);
  // Layer Group
